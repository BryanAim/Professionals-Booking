...
from professional_service_admin.models import ServiceDepartment, specialization, service, Test_Information
...
@csrf_exempt
def professional_service_home(request):
    ...
    context = {'professionals': professionals, 'professional_services': professional_services} 
...
@csrf_exempt
@login_required(login_url="login")
def multiple_professional_service(request):
    ...
    if request.user.is_authenticated: 
        ...
        if request.user.is_client:
            ...
            professional_services, search_query = searchProfessionalServices(request)
            
            # PAGINATION ADDED TO MULTIPLE HOSPITALS
            custom_range, professional_services = paginateProfessionalServices(request, professional_services, 3)
        
            context = {'client': client, 'professionals': professionals, 'professional_services': professional_services, 'search_query': search_query, 'custom_range': custom_range}
            return render(request, 'multiple-professional_service.html', context)
            ...
            context = {'professional': professional, 'professional_services': professional_services, 'search_query': search_query}
            return render(request, 'multiple-professional_service.html', context)
    else:
        logout(request)
        messages.error(request, 'Not Authorized')
...
def professional_service_profile(request, pk):
    ...
    if request.user.is_authenticated: 
        ...
        if request.user.is_client:
            ...
            professions = ServiceDepartment.objects.filter(professional_service=professional_services)
            ...
            context = {'client': client, 'professionals': professionals, 'professional_services': professional_services, 'professions': professions, 'specializations': specializations, 'services': services}
            ...
            context = {'professional': professional, 'professional_services': professional_services, 'professions': professions, 'specializations': specializations, 'services': services}
            return render(request, 'professional_service-profile.html', context)
    else:
        logout(request)
        messages.error(request, 'Not Authorized')
...
def ServiceType_list(request, pk):
    if request.user.is_authenticated: 
        ...
        if request.user.is_client:
            ...
            context = {'client': client, 'professionals': professionals, 'professional_services': professional_services, 'professions': professions}
            return render(request, 'professional_service-profession-list.html', context)
            ...
            context = {'professional': professional, 'professional_services': professional_services, 'professions': professions}
            return render(request, 'professional_service-profession-list.html', context)
    else:
        logout(request)
...
@csrf_exempt
@login_required(login_url="login")
def professional_service_professional_register(request, pk):
    if request.user.is_authenticated: 
        ...
        if request.user.is_professional:
            ...
            if request.method == 'POST':
                ...
                messages.success(request, 'ProfessionalService Registration Request Sent')
                
                return redirect('professional-dashboard')
                
                 
            context = {'professional': professional, 'professional_services': professional_services, 'professions': professions, 'specializations': specializations}
            return render(request, 'professional_service-professional-register.html', context)
    else:
        logout(request)
        messages.info(request, 'Not Authorized')
...
def testing(request):
    # professional_services = Professional_Service_Information.objects.get(professional_service_id=1)
    test = "test"
    context = {'test': test}
